...
AlgoVoi gateway returns a checkout URL + token
        │
        ▼
Reader redirected to api.algovoi.co.uk/checkout/{token}
        │
        ▼
...
Phase 2 (pay-then-grant-access) requires a real Ghost blog + a real AlgoVoi tenant + a real on-chain payment. Use the `your-domain.example` test blog once the Ghost side of the demo is wired up.
