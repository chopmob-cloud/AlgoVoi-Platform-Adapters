...
(0, 'payment_algovoi', 'payment_algovoi_api_base_url',        'https://api.algovoi.co.uk',        0),
...
3. Controller POSTs to `POST /v1/payment-links`:
   ```http
   POST https://api.algovoi.co.uk/v1/payment-links
...
- Proxies to `POST https://api.algovoi.co.uk/checkout/{token}/verify`.
...
Confirmed end-to-end on **2026-04-14** against `api.algovoi.co.uk`, OpenCart 4.1.0.3 at `your-domain.example`:
