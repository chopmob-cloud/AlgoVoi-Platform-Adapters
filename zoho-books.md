...
3. Under **Trigger**, select **Invoices → Created** (and optionally **Invoices → Sent**)
4. Under **Body Type**, select **JSON**
5. Add the following entity parameters to the body so the invoice total is included:
...
> **Important:** Without explicitly adding `${INVOICE.INVOICE_TOTAL}` to the webhook body, Zoho will not include the amount in the payload. AlgoVoi cannot generate a payment link without the invoice total.
