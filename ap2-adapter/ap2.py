...
class Ap2Gate:
    ...
    def _verify_mandate(self, mandate_data: dict) -> bool:
        """
        Verify an AP2 mandate by checking the ed25519 signature locally.

        Per the AP2 spec, verification is local — there is no central verify API.
        The server derives the agent's ed25519 public key from its Algorand address
        (Algorand addresses encode the 32-byte ed25519 public key directly), then
        verifies the signature over the canonical mandate message.

        Signing message: canonical JSON of mandate fields (excluding 'signature'),
        keys sorted, no spaces: {"amount":...,"merchant_id":...,"network":...,"payer_address":...}

        Requires either PyNaCl (nacl) or the cryptography package. Both are available
        if algosdk is installed (algosdk depends on PyNaCl).
        """
        ...
        # Algorand address = base32(ed25519_pubkey[0:32] + checksum[0:4])
        try:
            from algosdk import encoding as _enc  # type: ignore
            public_key_bytes = _enc.decode_address(payer_address)
        except Exception:
            # algosdk not available — fall back to raw base32 decode
            try:
        ...
        # Reconstruct canonical signing message (mandate fields minus signature)
        try:
            message_fields = {
                k: v for k, v in mandate_data.items() if k != "signature"
            }
        ...
        # Decode signature
        try:
            signature_bytes = b64decode(sig_b64)
        except Exception:
            return False

        # Verify with PyNaCl (preferred — available via algosdk)
        try:
