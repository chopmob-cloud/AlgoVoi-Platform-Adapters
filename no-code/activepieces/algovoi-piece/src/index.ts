...
**Tip:** Point the API Base URL at \`https://cloud.algovoi.co.uk\` to have AlgoVoi Cloud manage payouts centrally — no Tenant ID or payout addresses needed. For direct API access use \`https://api.algovoi.co.uk\` with all fields filled.
  `,
  props: {
    ...
    api_base: PieceAuth.ShortText({
      ...
      description: 'AlgoVoi API base URL. Leave blank for default (https://api.algovoi.co.uk).',
      required: false,
    }),
  },
  validate: async ({ auth }) => {
    const base = (auth.api_base || 'https://api.algovoi.co.uk').replace(/\/$/, '');
    try {
