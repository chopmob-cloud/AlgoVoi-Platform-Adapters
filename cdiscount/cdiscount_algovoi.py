"""
AlgoVoi Cdiscount / Octopia Payment Adapter

Polls the Octopia REST API for orders with status WaitingAcceptance,
accepts them, creates AlgoVoi payment links, and ships orders when
payment is confirmed on-chain.

Cdiscount/Octopia does not support push webhooks. This adapter polls
GET /orders?status=WaitingAcceptance, accepts via POST /orders/{id}/accept,
and ships via POST /orders/{id}/ship.
...
Octopia REST API docs: https://api.octopia-io.net/
...
_OCTOPIA_API = "https://api.octopia-io.net"
_OCTOPIA_AUTH = "https://auth.octopia.com/auth/realms/maas/protocol/openid-connect/token"


class CdiscountAlgoVoi:
    """Cdiscount / Octopia REST API + AlgoVoi payment adapter (polling-based)."""

    def __init__(
        self,
        api_base: str = "https://api.algovoi.co.uk",
        api_key: str = "",
        tenant_id: str = "",
        cdiscount_api_key: str = "",
        cdiscount_api_secret: str = "",
        seller_id: str = "",
        webhook_secret: str = "",
        ...
        self.cdiscount_api_key = cdiscount_api_key
        self.cdiscount_api_secret = cdiscount_api_secret
    ...
    def _refresh_token(self) -> bool:
        """Obtain a new access token via Octopia client_credentials flow."""
        if not self.cdiscount_api_key or not self.cdiscount_api_secret:
            return False

        body = urlencode({
            "grant_type": "client_credentials",
            "client_id": self.cdiscount_api_key,
            "client_secret": self.cdiscount_api_secret,
        }).encode()
    ...
    def _octopia_headers(self) -> dict:
        headers = {
        ...
        if self.seller_id:
            headers["SellerId"] = self.seller_id
        return headers
    ...
    def process_order(
        ...
        """
        Create an AlgoVoi payment link for a Cdiscount/Octopia order.

        Args:
            order_id:     Octopia order ID
            amount:       Order total
        ...
        if not label:
            label = f"Cdiscount Order #{order_id}"
    ...
    # ── Octopia API Integration ──────────────────────────────────────────

    def parse_order(self, data: dict) -> Optional[dict]:
        """
        Parse an Octopia order response into a normalised order dict.

        Args:
            data: Order JSON from GET /orders or GET /orders/{id}
        ...
        try:
            ...
            amount = float(data.get("totalAmount", data.get("amount", 0)))
            currency = data.get("currency", self.base_currency) or self.base_currency
            status = data.get("status", "WaitingAcceptance")
            buyer = data.get("buyer", data.get("customer", {}))
    ...
    def accept_order(self, order_id: str) -> bool:
        """
        Accept a WaitingAcceptance order on Octopia.
        ...
        url = f"{_OCTOPIA_API}/orders/{quote(str(order_id), safe='')}/accept"
        req = Request(url, data=b"{}", method="POST", headers=self._octopia_headers())
        try:
            with urlopen(req, timeout=self.timeout, context=self._ssl) as resp:  # nosec B310
    ...
    def poll_orders(self, since_datetime: Optional[str] = None) -> list:
        """
        Poll Octopia API for orders with WaitingAcceptance status.

        Args:
            since_datetime: ISO8601 timestamp (unused; Octopia uses status filter)
        ...
        url = f"{_OCTOPIA_API}/orders?status=WaitingAcceptance"
        try:
            req = Request(url, method="GET", headers=self._octopia_headers())
            with urlopen(req, timeout=self.timeout, context=self._ssl) as resp:  # nosec B310
                data = json.loads(resp.read())
        except (URLError, json.JSONDecodeError, OSError):
            return []

        # Octopia returns either a list or {"orders": [...]}
    ...
    def fulfill_order(self, order_id: str, tx_id: str) -> bool:
        """
        Ship a Cdiscount/Octopia order with the AlgoVoi TX reference.

        Uses POST /orders/{id}/ship.
        ...
        Returns:
            True if the ship call was accepted
        """
        ...
        url = f"{_OCTOPIA_API}/orders/{quote(str(order_id), safe='')}/ship"
