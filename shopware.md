...
| Network | Asset | Notes |
|---------|-------|-------|
| Webhook → checkout link | Webhook → checkout link | `algorand_mainnet` (USDC (ASA 31566704)) | Pass | Requires ASA opt-in on payout wallet |
...
| Test | Network | Result |
|------|---------|--------|
| `checkout.order.placed` webhook -> checkout link | Webhook → checkout link | Webhook → checkout link | `algorand_mainnet` (USDC (ASA 31566704)) | Pass |

Signature: `HMAC-SHA256` hex digest in `Shopware-Shop-Signature`. Amount field: `data.payload.amountTotal` (float, major units). Currency from `data.payload.currency.isoCode`.
