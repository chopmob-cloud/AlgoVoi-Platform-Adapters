...
Phase 2 — verify_payments(): takes real TX IDs, verifies on-chain, then calls
           the LlamaIndex OpenAI LLM and exercises the agent tool path.

Usage:
    python smoke_test_llamaindex.py                                     # Phase 1
    python smoke_test_llamaindex.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX  # Phase 2
...
def _load_openai_key() -> str:
    ...
    for fname in ("keys.txt", "openai.txt"):
        ...
        if v and v.startswith("sk-"):
            return v
    # Fall back to first bare sk- line
...
TEST_MESSAGES = [
    ...
    {"role": "user",   "content": "Payment verified. Say hello and confirm you are ChatGPT via LlamaIndex."},
]

# ── Gate helpers ──────────────────────────────────────────────────────────────

def _mpp_gate(network: str) -> AlgoVoiLlamaIndex:
    return AlgoVoiLlamaIndex(
...
def _ap2_gate(network: str) -> AlgoVoiLlamaIndex:
    return AlgoVoiLlamaIndex(
        openai_key        = OPENAI_KEY,
        algovoi_key       = ALGOVOI_KEY,
        tenant_id         = TENANT_ID,
...
def show_challenges():
    print("\n" + "=" * 60)
    print("PHASE 1 — Payment Challenges (LlamaIndex / ChatOpenAI via LlamaIndex)")
    print("=" * 60)
    ...
    print("  python smoke_test_llamaindex.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX")
...
def verify_payments(algo_tx: str, voi_tx: str, hedera_tx: str, stellar_tx: str):
    print("\n" + "=" * 60)
    print("PHASE 2 — On-chain Verification + LlamaIndex OpenAI LLM")
    print("=" * 60)
    ...
    for network, tx_id in tests:
        ...
        try:
            ...
            print(f"  [PASS] LlamaIndex LLM replied: {reply[:120]}")
            passed += 1
    ...
    # ── Agent tool phase ──────────────────────────────────────────────────────
    print(f"\n-- Agent tool (as_tool / __call__) on algorand-mainnet -----")
    try:
        gate  = _mpp_gate("algorand-mainnet")
        proof = _mpp_proof("algorand-mainnet", algo_tx)

        resource_fn = lambda q: f"Protected LlamaIndex answer to: {q}"
        tool = gate.as_tool(resource_fn=resource_fn, tool_name="demo_gate")

        # No proof — should return challenge JSON
        import json as _json
        tool_input_no_proof = _json.dumps({"query": "hello"})
        tool_output_no_proof = tool(tool_input_no_proof)
        challenge_data = _json.loads(tool_output_no_proof.content)
        assert challenge_data.get("error") == "payment_required", (
            f"Expected payment_required challenge, got: {challenge_data}"
        )
        print("  [PASS] Tool __call__ returns challenge ToolOutput when proof absent")

        # With proof — should call resource_fn
        tool_input_with_proof = _json.dumps({"query": "hello", "payment_proof": proof})
        tool_output_with_proof = tool(tool_input_with_proof)
        assert tool_output_with_proof.content == "Protected LlamaIndex answer to: hello", (
            f"Unexpected content: {tool_output_with_proof.content}"
        )
        print(f"  [PASS] Tool returned ToolOutput: {tool_output_with_proof.content}")
        print(f"         tool_name: {tool_output_with_proof.tool_name}")
        passed += 1
...
if __name__ == "__main__":
    if len(sys.argv) == 5:
        ...
        print("Usage:")
        print("  python smoke_test_llamaindex.py                              # Phase 1")
        print("  python smoke_test_llamaindex.py ALGO VOI HEDERA STELLAR     # Phase 2")
        sys.exit(1)
