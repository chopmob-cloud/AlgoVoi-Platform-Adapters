...
def scene_402(adapter: str, model: str, protocol: str,
    ...
    PROTOCOL_NOTES = {
        "mpp":  ("MPP — IETF Payment Auth (WWW-Authenticate)",
                 ["Your Flask server calls api.algovoi.co.uk internally to issue",
...
def scene_200(adapter: str, model: str, ai_reply: str) -> list[Image.Image]:
    ...
    frames += _annotation_frames(
        ...
        lines=[
            "api.algovoi.co.uk verified the TX on-chain in real time.",
...
def scene_howto_code() -> list[Image.Image]:
    ...
    frames += _annotation_frames(
        ...
        lines=[
            "gate.check()  →  verifies payment proof via api.algovoi.co.uk.",
...
def scene_howto_run(ai_reply: str, challenge_header: str = "") -> list[Image.Image]:
    ...
    frames += _annotation_frames(
        ...
        lines=[
            "api.algovoi.co.uk issues a real X-PAYMENT-REQUIRED challenge.",
