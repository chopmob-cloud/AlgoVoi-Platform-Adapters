"""
xAI (Grok) Adapter Smoke Test
=============================
Phase 1 — show_challenges(): shows the 402 payment challenges for MPP + AP2 on all 4 chains
Phase 2 — verify_payments(): takes real TX IDs, verifies on-chain, calls xAI API

Usage:
    python smoke_test_xai.py                              # Phase 1 only
    python smoke_test_xai.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX   # Phase 2

Credentials:
    XAI_KEY      env var (or first line starting with "xai-" in xai.txt / openai.txt)
...
def _load_xai_key() -> str:
    k = os.environ.get("XAI_KEY") or os.environ.get("XAI_API_KEY")
    if k:
        return k
    repo_root = os.path.join(os.path.dirname(__file__), "..", "..")
    for candidate in ("xai.txt", "openai.txt"):
        p = os.path.join(repo_root, candidate)
        v = _load_line("xai-", p)
        if v:
            return v
    raise RuntimeError(
        "XAI_KEY not set. Either export XAI_KEY=... or add a line starting "
        "with 'xai-' to xai.txt in the repo root."
    )
...
TEST_MESSAGES = [
    ...
    {"role": "user",   "content": "Payment verified. Say hello and confirm you are Grok."},
]

# -- Gate helpers --------------------------------------------------------------

def _mpp_gate(network: str) -> AlgoVoiXai:
    return AlgoVoiXai(
        xai_key           = XAI_KEY,
...
def _ap2_gate(network: str) -> AlgoVoiXai:
    return AlgoVoiXai(
        xai_key           = XAI_KEY,
        algovoi_key       = ALGOVOI_KEY,
        tenant_id         = TENANT_ID,
...
def show_challenges():
    print("\n" + "="*60)
    print("PHASE 1 — Payment Challenges (xAI / Grok)")
    print("="*60)
    ...
    print("  python smoke_test_xai.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX")
...
def verify_payments(algo_tx: str, voi_tx: str, hedera_tx: str, stellar_tx: str):
    print("\n" + "="*60)
    print("PHASE 2 — On-chain Verification + xAI API")
    print("="*60)
    ...
    for network, tx_id, proof_network in tests:
        ...
        try:
            ...
            print(f"  [PASS] Grok replied: {reply[:120]}")
            passed += 1
...
if __name__ == "__main__":
    if len(sys.argv) == 5:
        ...
        print("Usage:")
        print("  python smoke_test_xai.py                              # Phase 1")
        print("  python smoke_test_xai.py ALGO VOI HEDERA STELLAR     # Phase 2")
        sys.exit(1)
