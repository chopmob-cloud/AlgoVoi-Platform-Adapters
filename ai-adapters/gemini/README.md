# Gemini Adapter for AlgoVoi

Payment-gate the Google Gemini API using x402, MPP, or AP2 — paid in USDC on Algorand, VOI, Hedera, or Stellar.

**v1.0.0 — 75/75 tests passing. Payment verification smoke-tested on all 4 chains 14 Apr 2026.**

Full integration guide: [ai-adapters/gemini/](.)
...
Client sends request
        |
        v
AlgoVoiGemini.check() -- no payment proof
        |
        v
...
Client sends proof in request header
        |
        v
AlgoVoiGemini.check() -- proof verified
        |
        v
AlgoVoiGemini.complete() calls Google Gemini API
        |
        v
HTTP 200 -- Gemini response returned
```
...
| File | Description |
|------|-------------|
| `gemini_algovoi.py` | Adapter — `AlgoVoiGemini`, `GeminiAiResult`, gate factory |
| `test_gemini_algovoi.py` | 75 unit tests (all mocked, no live calls) |
| `smoke_test_gemini.py` | MPP live smoke test — all 4 chains |
...
Accepts OpenAI-format message lists. The `system` role is extracted automatically into Gemini's `GenerateContentConfig(system_instruction=...)`, and `assistant` roles are mapped to `model` internally — no conversion needed on your side:
...
|-----------|------|---------|-------------|
| `gemini_key` | str | required | Google Gemini API key (`AIza...`) |
...
| `gemini-2.0-flash` | Balanced — default |
...
> **Note:** Uses the new `google-genai` SDK. The older `google-generativeai` package is deprecated and is not supported.
