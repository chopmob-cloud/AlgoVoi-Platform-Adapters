...
Phase 2 — verify_payments(): takes real TX IDs, verifies on-chain, calls Claude API

Usage:
    python smoke_test_claude.py           # Phase 1 only (show challenges)
    python smoke_test_claude.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX   # Phase 2
"""
...
def _load_key(prefix: str) -> str:
    """Read the first line starting with `prefix` from openai.txt."""
...
def _mpp_gate(network: str) -> AlgoVoiClaude:
    return AlgoVoiClaude(
...
def _ap2_gate(network: str) -> AlgoVoiClaude:
    return AlgoVoiClaude(
        anthropic_key     = ANTHROPIC_KEY,
        algovoi_key       = ALGOVOI_KEY,
        tenant_id         = TENANT_ID,
...
def show_challenges():
    ...
    print("  python smoke_test_claude.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX")
...
def verify_payments(algo_tx: str, voi_tx: str, hedera_tx: str, stellar_tx: str):
    print("\n" + "="*60)
    print("PHASE 2 — On-chain Verification + Claude API")
    print("="*60)
    ...
    for network, tx_id, proof_network in tests:
        ...
        try:
            ...
            print(f"  [PASS] Claude replied: {reply[:120]}")
            passed += 1
...
if __name__ == "__main__":
    if len(sys.argv) == 5:
        ...
        print("Usage:")
        print("  python smoke_test_claude.py                              # Phase 1")
        print("  python smoke_test_claude.py ALGO VOI HEDERA STELLAR     # Phase 2")
        sys.exit(1)
