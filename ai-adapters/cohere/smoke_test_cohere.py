"""
Cohere Adapter Smoke Test
=========================
Phase 1 — show_challenges(): shows the 402 payment challenges for MPP + AP2 on all 4 chains
Phase 2 — verify_payments(): takes real TX IDs, verifies on-chain, calls Cohere API

Usage:
    python smoke_test_cohere.py                              # Phase 1 only
    python smoke_test_cohere.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX   # Phase 2

Credentials:
    COHERE_KEY   env var (or first line starting with "co-" / "Z" in openai.txt)
    ALGOVOI_KEY  env var (or first "algv_" line in openai.txt)
...
def _load_line(prefix: str, path: str) -> str | None:
    try:
...
def _load_cohere_key() -> str:
    # 1. env var
    k = os.environ.get("COHERE_KEY") or os.environ.get("CO_API_KEY")
    if k:
        return k
    # 2. cohere.txt next to the repo root
    repo_root = os.path.join(os.path.dirname(__file__), "..", "..")
    for candidate in ("cohere.txt", "openai.txt"):
        p = os.path.join(repo_root, candidate)
        for prefix in ("co-", "Z", ""):  # Cohere keys have no fixed prefix
            if not prefix:
                continue  # skip empty — would match any line
            v = _load_line(prefix, p)
            if v and not v.startswith("sk-") and not v.startswith("AIza") \
                    and not v.startswith("algv_") and not v.startswith("main"):
                return v
    raise RuntimeError(
        "COHERE_KEY not set. Either export COHERE_KEY=... or add a line to "
        "cohere.txt in the repo root."
    )
...
def _load_algovoi_key() -> str:
    ...
    if k:
        return k
    repo_root = os.path.join(os.path.dirname(__file__), "..", "..")
    v = _load_line("algv_", os.path.join(repo_root, "openai.txt"))
    if v:
        return v
    v = _load_line("algv_", os.path.join(repo_root, "keys.txt"))
    if v:
...
TEST_MESSAGES = [
    ...
    {"role": "user",   "content": "Payment verified. Say hello and confirm you are Cohere."},
]

# -- Gate helpers --------------------------------------------------------------

def _mpp_gate(network: str) -> AlgoVoiCohere:
    return AlgoVoiCohere(
...
def _ap2_gate(network: str) -> AlgoVoiCohere:
    return AlgoVoiCohere(
        cohere_key        = COHERE_KEY,
        algovoi_key       = ALGOVOI_KEY,
        tenant_id         = TENANT_ID,
...
def show_challenges():
    print("\n" + "="*60)
    print("PHASE 1 — Payment Challenges (Cohere)")
    print("="*60)
    ...
    print("  python smoke_test_cohere.py ALGO_TX VOI_TX HEDERA_TX STELLAR_TX")
...
def verify_payments(algo_tx: str, voi_tx: str, hedera_tx: str, stellar_tx: str):
    print("\n" + "="*60)
    print("PHASE 2 — On-chain Verification + Cohere API")
    print("="*60)
    ...
    for network, tx_id, proof_network in tests:
        ...
        try:
            ...
            print(f"  [PASS] Cohere replied: {reply[:120]}")
            passed += 1
...
if __name__ == "__main__":
    if len(sys.argv) == 5:
        ...
        print("Usage:")
        print("  python smoke_test_cohere.py                              # Phase 1")
        print("  python smoke_test_cohere.py ALGO VOI HEDERA STELLAR     # Phase 2")
        sys.exit(1)
