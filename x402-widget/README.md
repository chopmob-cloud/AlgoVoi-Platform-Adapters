...
Live demo: [your-domain.example](https://your-domain.example)
...
| `api-url`   | No       | `https://your-domain.example/api/x402/pay`         | Override to point at your own backend proxy               |
...
Widget POSTs { chain, amount, currency } to api-url
        ↓
Cloudflare Function proxies to POST /v1/payment-links on api.algovoi.co.uk
        ↓
...
export async function onRequestPost(context) {
  ...
  const res = await fetch('https://api.algovoi.co.uk/v1/payment-links', {
    method: 'POST',
    headers: {
...
app.post('/api/create-payment', async (req, res) => {
  ...
  const response = await fetch('https://api.algovoi.co.uk/v1/payment-links', {
    method: 'POST',
    headers: {
      'Content-Type':  'application/json',
      'Authorization': `Bearer ${process.env.ALGOVOI_API_KEY}`,
...
The reference deployment is at `https://your-domain.example`:

- Demo page: `https://your-domain.example`
- Widget JS: `https://your-domain.example/widget.js`
- Demo endpoint: `POST https://your-domain.example/api/x402/demo`
- Pay endpoint: `POST https://your-domain.example/api/x402/pay`
