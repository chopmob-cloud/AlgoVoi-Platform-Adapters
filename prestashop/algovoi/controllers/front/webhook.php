 ...
 */
class AlgovoiWebhookModuleFrontController extends ModuleFrontController
{
    public function postProcess()
    {
        $raw_body   = file_get_contents("php://input");
        $secret     = Configuration::get("ALGOVOI_WEBHOOK_SECRET");
        ...
        if (!empty($data["label"]) && preg_match("/Cart #(\d+)/", $data["label"], $m)) {
            ...
            if ($order_id) {
                ...
                if (Validate::isLoadedObject($order) && in_array($order->current_state, [
                    (int)Configuration::get("ALGOVOI_PENDING_STATUS"), 1, 2,
                ])) {
