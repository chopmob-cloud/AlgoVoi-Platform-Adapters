...
class Algovoi_Ext extends PaymentModule
{
    public function __construct()
    {
        ...
        $this->version     = "2.4.1";
        $this->author      = "AlgoVoi";
        $this->author_uri  = "https://api.algovoi.co.uk";
        $this->need_instance = 0;
        $this->bootstrap   = true;
        $this->ps_versions_compliancy = ['min' => '8.0.0', 'max' => _PS_VERSION_];
        parent::__construct();
        $this->displayName = $this->l("AlgoVoi Wallet Checkout");
        $this->description = $this->l("Pay with USDC (Algorand) or aUSDC (VOI) directly from your browser wallet via the AlgoVoi extension. No redirect — pay in-page instantly.");
    }
    ...
    public function install()
    {
        return parent::install()
            ...
            && Configuration::updateValue("ALGOVOI_EXT_API_BASE_URL",   "https://api.algovoi.co.uk")
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $actionUrl = $this->context->link->getModuleLink("algovoi_ext", "payment", [], true);
        $chainSelector = '
        ...
        <div style="display:flex;gap:16px;margin-bottom:.25rem;">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;">
            <input type="radio" name="algovoi_ext_network_radio" value="algorand_mainnet" checked
                   onchange="document.querySelector(\'input[name=algovoi_ext_network]\').value=this.value;"> Algorand &mdash; USDC
          </label>
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;">
            <input type="radio" name="algovoi_ext_network_radio" value="voi_mainnet"
                   onchange="document.querySelector(\'input[name=algovoi_ext_network]\').value=this.value;"> VOI &mdash; aUSDC
          </label>
        </div>';
        $option = new PaymentOption();
        $option->setCallToActionText($this->l("Pay with AlgoVoi Extension (USDC / aUSDC)"))
               ->setAction($actionUrl)
               ->setAdditionalInformation($chainSelector)
               ->setInputs([
                   ['type' => 'hidden', 'name' => 'algovoi_ext_network', 'value' => 'algorand_mainnet'],
               ]);
        return [$option];
    }
