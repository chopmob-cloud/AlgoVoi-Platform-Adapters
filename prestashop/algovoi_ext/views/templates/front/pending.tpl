...
<div id="av-ext-pay" style="max-width:520px;margin:2rem auto;padding:1.5rem 1.75rem;background:#1e2130;border:1px solid #2a2d3a;border-radius:12px;color:#f1f2f6;font-family:system-ui,sans-serif;">
  ...
  <p style="margin:0 0 1.25rem;color:#9ca3af;font-size:.9rem;line-height:1.6;">
    Send <strong style="color:#10b981;">{$algovoi_amount|escape:'html':'UTF-8'} {$algovoi_ticker|escape:'html':'UTF-8'}</strong>
    on <strong style="color:#f1f2f6;">{$algovoi_chain|escape:'html':'UTF-8'}</strong> using the AlgoVoi browser extension.
  </p>
  ...
  <div id="av-no-ext" style="display:none;margin-bottom:1rem;padding:.75rem 1rem;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:8px;font-size:.85rem;color:#ef4444;">
    ...
    <a href="{$algovoi_checkout_url|escape:'html':'UTF-8'}" target="_blank" rel="noopener" style="color:#3b82f6;">Pay on hosted checkout &rarr;</a>
  </div>
  ...
  <button id="av-pay-btn" onclick="avPayWithExtension()"
    ...
    &#9889; Pay {$algovoi_amount|escape:'html':'UTF-8'} {$algovoi_ticker|escape:'html':'UTF-8'} via Extension
  </button>

  <p style="margin:.85rem 0 0;font-size:.75rem;color:#6b7280;">
    No extension? <a href="{$algovoi_checkout_url|escape:'html':'UTF-8'}" target="_blank" rel="noopener" style="color:#3b82f6;">Pay on the hosted checkout page</a> instead.
  </p>
</div>
...
(function () {
  var AV = {
    receiver:   {$algovoi_receiver|json_encode nofilter},
    memo:       {$algovoi_memo|json_encode nofilter},
    microunits: {$algovoi_amount_mu|intval},
    assetId:    {$algovoi_asset_id|intval},
    algodUrl:   {$algovoi_algod_url|json_encode nofilter},
    verifyUrl:  {$algovoi_verify_url|json_encode nofilter},
    successUrl: {$algovoi_success_url|json_encode nofilter},
  };
  ...
  function setBtn(txt, dis) {
    ...
    if (!b) return;
    b.textContent = txt; b.disabled = !!dis; b.style.opacity = dis ? '.6' : '1';
  }
  ...
  window.avPayWithExtension = async function () {
    try {
      ...
      var submitResp = await fetch(AV.algodUrl + '/v2/transactions', {
        method: 'POST', headers: { 'Content-Type': 'application/x-binary' }, body: stxnBytes,
      });
      ...
      if (!txId) throw new Error('No txId in response: ' + JSON.stringify(submitData));
      ...
      for (var attempt = 0; attempt < 20; attempt++) {
        ...
        if (pendingData['confirmed-round'] && pendingData['confirmed-round'] > 0) { confirmedRound = pendingData['confirmed-round']; break; }
        if (pendingData['pool-error'] && pendingData['pool-error'].length > 0) throw new Error('Transaction rejected: ' + pendingData['pool-error']);
      }
      ...
      if (vr.ok && vd.success) {
        showMsg('\u2713 Payment verified! Redirecting\u2026', 'ok');
        setBtn('Paid \u2713', true);
        setTimeout(function () { location = AV.successUrl; }, 2000);
      } else {
        throw new Error(vd.error || vd.detail || 'Verification failed.');
      }
    } catch (err) {
